...
| **Inspect Scout** | Quantitative transcript analysis via scanners (refusal detection, eval awareness, custom LLM/pattern scanners). Works on Inspect logs or any other transcript data (non-Inspect formats may need a conversion step; check the docs). | https://meridianlabs-ai.github.io/inspect_scout/llms.txt |
