...
| Phase | Cadence | Anchor (from `task list --json` + `sample list --json`) |
|---|---|---|
| Spinup (provider auth, dataset fetch, sandbox build; where most failures live) | ~1 min | `counts.completed == 0` |
...
- `task list`: running tasks across all live processes. Each row carries the selectors other commands take (`task_id`, `pid`) plus `log_location`, `last_activity_at`, `completed_at`, and `keep_alive` (`--json` shows fields the human table hides).
...
- `sample list [<task>]`: per-sample rows (status, retries, score, time, tokens, and `idle` since last activity). An omitted TASK spans all running tasks. `--active-since <ts>` is the "what changed since I last looked" delta: feed it the `as_of` from the prior response's envelope. The listing is capped with running samples first; `counts` is always the complete status histogram and `truncated` reports whether rows were dropped (if a delta comes back truncated, re-poll the same value with `--all` before advancing to the new `as_of`). Widen with `--limit N` / `--all`, narrow with `--status <s1,s2>`. A sample doesn't appear here until it has emitted at least one event, so a sample still in solver setup can be in flight per `task list` without showing up here; the polling rule above still applies (don't loop, come back to it when the user asks for an update).
...
- `process keep [PID]`: turn ON the post-completion park for a process launched WITHOUT `--ctl-server=keep`. The process will stay inspectable after its eval finishes instead of exiting. Does NOT change the running eval. Useful when you realize mid-run that you want to interrogate the eval after it completes.
